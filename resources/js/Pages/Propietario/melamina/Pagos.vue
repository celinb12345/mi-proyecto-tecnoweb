...
async function onTipoChange() {
  ...
  if (pago.value.tipo_pago === 'trabajador') {
    const r = await axios.get('propietario/melamina/pagos/trabajadores')
    trabajadores.value = r.data
    proveedores.value = []
    pago.value.id_proveedor = ''
  } else if (pago.value.tipo_pago === 'proveedor') {
    const r = await axios.get('propietario/melamina/pagos/proveedores')
...
async function registrarPago() {
  try {
    ...
    const res = await axios.post('propietario/melamina/pagos', payload)
...
async function listarPagos() {
  const r = await axios.get('propietario/melamina/pagos')
  pagos.value = r.data.pagos || []
}

async function buscarPagos() {
  const params = new URLSearchParams(filtro.value).toString()
  const r = await axios.get(`propietario/melamina/pagos?${params}`)
  pagos.value = r.data.pagos || []
}
...
async function cargarCuotasPendientes() {
  try {
    const r = await axios.get('propietario/melamina/pagos/cuotas-pendientes')
