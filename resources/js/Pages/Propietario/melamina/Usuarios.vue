    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Ej: cli_m1, trab_m2, prov_m3..."
        />
        <p
          class="text-xs mt-1"
...
async function registrarUsuario () {
  ...
  try {
    ...
    const res = await axios.post('propietario/melamina/usuarios', payload)
...
async function actualizarUsuario () {
  ...
  try {
    ...
    const res = await axios.put(
      `propietario/melamina/usuarios/${form.value.id_usuario}`,
      payload
    )
...
async function toggleEstado (u) {
  try {
    const res = await axios.patch(
      `propietario/melamina/usuarios/${u.id_usuario}/estado`
    )
    if (res.data.success) {
...
async function eliminarUsuario (u) {
  ...
  try {
    const res = await axios.delete(
      `propietario/melamina/usuarios/${u.id_usuario}`
    )
    if (res.data.success) {
      mensaje.value = res.data.message
      mensajeColor.value = 'text-green-600'
