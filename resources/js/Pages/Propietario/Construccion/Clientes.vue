...
const usuariosDisponibles = computed(() => {
  ...
  return usuarios.value.filter(u => {
    const esCliente = u.id_usuario.startsWith('cli_c')
...
async function listarClientes(params = {}) {
  try {
    const res = await axios.get('propietario/Construccion/clientes', { params })
...
async function registrarCliente() {
  try {
    const res = await axios.post('propietario/Construccion/clientes', form.value)
    if (res.data.success) {
...
async function actualizarCliente() {
  try {
    ...
    const res = await axios.put(`propietario/Construccion/clientes/${id}`, payload)
...
async function eliminarCliente(c) {
  ...
  try {
    const res = await axios.delete(`propietario/Construccion/clientes/${c.id_cliente}`)
    if (res.data.success) {
      mensaje.value = res.data.message
      mensajeColor.value = 'text-green-600'
      if (mostrarLista.value) await listarClientes()
